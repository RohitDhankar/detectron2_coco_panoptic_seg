...
class panoptic_fpn_std_data_dicts():
    def __init__(self):
        ...
        self.get_config.merge_from_file(model_zoo.get_config_file("COCO-PanopticSegmentation/panoptic_fpn_R_101_3x.yaml"))
        ##COCO-PanopticSegmentation/panoptic_fpn_R_101_3x"
        ## model_final_cafdb1.pkl: 261MB [00:22, 11.5MB/s]   

        self.get_config.MODEL.WEIGHTS = model_zoo.get_checkpoint_url("COCO-PanopticSegmentation/panoptic_fpn_R_101_3x.yaml")
    ...
    def get_panoptic_viz(self,testing_image_path,str_score_thresh_for_viz): #coco_image_forViz,coco_data_metaData,
        """
        PRED - panoptic Masks with the -- default_predictor
        ...
        """
        str_score_thresh_for_viz = "0.50"
        #testing_image_path = "./coco_val_images_2017/val2017/000000554002.jpg"  
        #testing_image_path = "/home/dhankar/temp/11_22/a___own_git_up/detect2/Obj_Detect_Detectron2/coco_val_images_2017/val2017/000000000139.jpg"

        ## Database : "The Penn-Fudan-Pedestrian Database"
        testing_image_path = "/home/dhankar/temp/04_22/data_dump/PennFudanPed/PNGImages/"
        #ls_people_images = ["FudanPed00001.png","FudanPed00002.png","FudanPed00003.png","FudanPed00043.png","FudanPed00048.png","FudanPed00047.png"]
        ls_people_images = ["FudanPed00004.png","FudanPed00005.png","FudanPed00006.png","FudanPed00007.png","FudanPed00008.png","FudanPed00009.png"]
        for iter_pan_img in range(len(ls_people_images)):

            print("-[INFO-get_panoptic_viz]-str(ls_people_images[iter_pan_img]-",str(ls_people_images[iter_pan_img]))

            testing_image_path1 = testing_image_path + str(ls_people_images[iter_pan_img])
            test_image = cv2.imread(testing_image_path1)
            #image_file_local_1 = testing_image_path.split(".jpg")[0]
            image_file_local_1 = testing_image_path1.split(".png")[0]
            image_file_local_2 = image_file_local_1.rsplit("/",1)[1]

            print("-[INFO-get_panoptic_viz]---image_file_local_2--",str(image_file_local_2))
            # default_predictor = self.predictor(test_image)
            panoptic_seg, segments_info = self.predictor(test_image)["panoptic_seg"]
            
            visualizer = Visualizer(test_image,metadata = MetadataCatalog.get(self.get_config.DATASETS.TRAIN[0]))#,
            coco_panoptic_img_viz = visualizer.draw_panoptic_seg_predictions(panoptic_seg.to("cpu"), segments_info)
            ...
            cv2.imwrite(save_coco_img_path+str(save_coco_img)+"_.png",coco_panoptic_img_viz.get_image())
...
#         SOURCE -- https://detectron2.readthedocs.io/en/latest/tutorials/models.html#load-save-a-checkpoint

#         EventStorage >> 
#         SOURCE -- https://detectron2.readthedocs.io/en/latest/tutorials/models.html#use-a-model
#         QUOTE -- >> When in training mode, all models are required to be used under an EventStorage. 
...
#     model = build_model(cfg_launch) ##  build_model >> returns a torch.nn.Module >> <class 'torch.nn.parallel.distributed.DistributedDataParallel'>
...
if __name__ == "__main__":
    ...
    obj_class_1 = panoptic_fpn_std_data_dicts() 
    
    obj_class_1.get_panoptic_viz(testing_image_path="",str_score_thresh_for_viz="")
